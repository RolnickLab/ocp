...
class BaseTrainer(ABC):
    ...
    def eval_all_splits(
        ...
        if len(dense_splits) != len(all_splits):
        ...
        if not self.silent:
            ...
            if (self.config["validate_dense"]) and (
                len(dense_splits) != len(all_splits)
            ):
                if final:
                    ...
                    highlights = set()  # {"energy_mae", "forces_mae", "total_loss"}
                    smn = sorted(
                        [m if "loss" not in m else f"z_{m}" for m in metrics_names]
                    )
                    for metric in smn:
                        metric = metric[2:] if metric.startswith("z_") else metric
                        row = [metric] + [
                        ...
                        table_dense.add_row(
                            *row, style="on white" if metric in highlights else ""
                        )
